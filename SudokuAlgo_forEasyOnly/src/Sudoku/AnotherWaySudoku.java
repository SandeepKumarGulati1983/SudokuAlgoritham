...
public class AnotherWaySudoku {
		...
		for (int row = 0; row < 9; row++) {
			for (int col = 0; col < 9; col++) {
				if (!this.operationalSudokuMatrix[row][col].status) {
					log.sleepAndShowStatus("<<reach to empty cell  >>", "This cell is not filled ---", row, col);
					// 2. scan row/col/ cube and then assign one value from possible candidates.
					getPossibleCandidates(this.operationalSudokuMatrix, row, col);
					fillUniqueCandidateAndFillCell(this.operationalSudokuMatrix);
					assignOneValue(this.operationalSudokuMatrix, row, col);
	...
	public void getPossibleCandidates(Cell[][] matrix, int row, int col) {
		...
		//assignIfPossibleCandidatesAreOne(matrix[row][col]);
		log.sleepAndShowStatus("<<assigning finally to the cell >>", "", row, col);

	}

	void assignIfPossibleCandidatesAreOne(Cell cell){
		if (cell.possibleCandidtes.size() ==1) {
			cell.cellValue = cell.possibleCandidtes.getFirst();
			cell.isUniqueCandidateAvailable = true;
			log.sleepAndShowStatus("<<only one uniqe element present for this cell >>", "" + cell.cellValue,
					11, 11);
			cell.status = true;
		}
	}
	...
	void assignOneValue(Cell[][] matrix, int row, int col) {

		matrix[row][col].cellValue = matrix[row][col].possibleCandidtes.getFirst();
		matrix[row][col].isUniqueCandidateAvailable = true;
		log.sleepAndShowStatus("<<filling value to this cell --- >>", "" + matrix[row][col].cellValue, row, col);
...
System.out.println("size of the list  ----"+blockElement.possibleCandidtes);
